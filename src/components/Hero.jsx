...
const Hero = () => {
  ...
  // Set initial states before paint to prevent flash (we avoid relying on opacity here
  // because elements are already hidden via inline style in the JSX)
  useLayoutEffect(() => {
    // Background vectors — set transforms (no need to set opacity here)
    ...
    // Profile
    gsap.set(elementsRef.current[0], { scale: 0.75, rotationY: 35 });

    // Intro
    gsap.set(elementsRef.current[1], { y: 18, rotationX: 18 });

    // Heading words (we already set opacity:0 inline on each .word)
    ...
    // Paragraph
    gsap.set(elementsRef.current[3], { y: 28 });

    // Button
    gsap.set(elementsRef.current[4], { y: 14, scale: 0.94 });
  }, []);
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.to(
        ...
        { opacity: 1, x: 0, rotationY: 0, scale: 1, duration: 1.6 }
      )
        .to(
      ...
      // Heading words (animate individual words)
      tl.to(
        headingSpans,
        { opacity: 1, y: 0, rotationX: 0, duration: 1.1, stagger: 0.12, ease: "power4.out" },
        1.4
      );
  ...
  return (
    ...
    >
      {/* Background vectors (start hidden via inline style) */}
      <div
        ref={vectorRightRef}
      ...
      <div
        ref={vectorLeftRef}
        className="hidden xl:flex absolute left-0 bottom-0"
        style={{ opacity: 0 }}
      >
      ...
      <div className="min-h-[95vh] sm:min-h-[80vh] lg:min-h-[100vh] flex items-center justify-center">
        <div className="mt-18 pb-4">
          <div className="grid w-[95%] m-auto place-items-center text-center gap-y-2">
            {/* Profile (container starts hidden) */}
            <div ref={(el) => (elementsRef.current[0] = el)} style={{ opacity: 0 }}>
              <img
              ...
              <div ref={(el) => (elementsRef.current[2] = el)} className="md:leading-[110%] leading-[220%]">
                <h1 className="font-semibold text-[30px] xs:text-4xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-5xl 2xl:text-[6xl] tracking-tighter text-gray-900 dark:text-white md:space-y-3">
                  {["Designing", "Digital"].map((word, i) => (
                    <span key={i} className="word inline-block mr-1" style={{ opacity: 0 }}>
                      {word}
                    </span>
                  ))}
                  <br />
                  ...
                  <span className="word inline-block mr-1" style={{ opacity: 0 }}>Communicate</span>
                </h1>
              </div>

              {/* Paragraph (starts hidden) */}
              <div ref={(el) => (elementsRef.current[3] = el)} style={{ opacity: 0 }}>
            ...
            <div ref={(el) => (elementsRef.current[4] = el)} className="flex gap-4 justify-center mt-4" style={{ opacity: 0 }}>
